...
void sigint_handler(int sig, siginfo_t* data, void* context) {
  //NOT ASYNC-SAFE to print!!
  printf("Process %i received signal %i sent from process %i\n", getpid() , sig, data->si_pid);
  printf("Process %i exiting due to signal %i\n", getpid(), sig);
  exit(0);
}
...
void recurse_minsmax_helper(int* data, int num_proc, int data_length) {
  ...
  //Don't need these because parent must have done already
  // 
...
struct stats main_iter_recur_minsmax(char* file, int proc_breadth, int proc_depth) {
  // Signal handlers for sendings data
  struct sigaction minsmax_sigact;
  minsmax_sigact.sa_flags = SA_SIGINFO;
  minsmax_sigact.sa_sigaction = &minsmax_signal_handler;
  ...
  minsmax_sigact.sa_sigaction = &sigint_handler;
  sigaction(SIGINT, &minsmax_sigact , NULL);

  // Set handlers for new signals here 
  // sigaction(SIGINT, &minsmax_sigact , NULL);
